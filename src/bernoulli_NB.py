...
#==============================================================================
bernoulli_NB.py
/Users/aelshen/Documents/Dropbox/School/CLMS 2013-2014/Winter 2014/Ling 572-Advanced Statistical Methods for NLP/hw3_572_aelshen/src/bernoulli_NB.py
...
class multinomial_NB:
    ...
    def Train(self):
        ...
        for line in self.train_file.readlines():
            ...
            for attribute in line[1:]:
                ...
                if count > 0:
                    ...
                    #increment the count of documents in this class that 
                    #have this feature
                    self.feature_count_per_class[label][feature] += 1
        ...
        for cls in self.classes:
            for word in self.vocabulary:
                ...
                self.cond_prob[word][cls] = self.FindBernoulliCondProb(word, cls)
        self.train_file.seek(0)
    
    def FindBernoulliCondProb(self, word, cls):
        p_wc = float( (self.cond_prob_delta + self.feature_count_per_class[cls][word]) ) \
               / (2 * self.cond_prob_delta + self.classes[cls])
        return p_wc
    ...
    def Test(self):
        ...
        for line in self.train_file.readlines():
            ...
            for attribute in line[1:]:
                ...
                if count > 0:
                    features_in_current_doc.add(word)
            #end for attribute in line[1:]: 
            
            for word in self.vocabulary:
                for cls in self.classes:
                    #if the current word was observed as being in this class, 
                    #the log of the conditional probability will be added.
                    if word in features_in_current_doc:
                        class_prob[cls] += math.log10( self.cond_prob[word][cls] )
                    #if not, the probability of the word not present for this class
                    #will be added instead
                    else:
                        class_prob[cls] += math.log10(1 - self.cond_prob[word][cls])
        ...
        for line in self.test_file.readlines():
            ...
            for word in self.vocabulary:
                ...
                #end for cls in self.classes: 
            #end for word in self.vocabulary:
    ...
    def PrintModel(self):
        ...
        for cls in self.classes:
            ...
            for word in sorted(self.vocabulary):
                val = self.cond_prob[word][cls]
                self.model_file.write(word + "\t" + cls + "\t" + str( val ) + "\t")
                if val == 0:
                    self.model_file.write("-inf" + os.linesep)
                else:
                    self.model_file.write( str( math.log10( val ) ) + os.linesep)
    ...
    def PrintSys(self):
        ...
        #the above repeated once more for test data           
